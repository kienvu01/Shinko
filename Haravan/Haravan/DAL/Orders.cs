...
namespace Haravan.Haravan.DataAcess
{
    public class Orders
    {
        ...
        {
            string token = MyAppData.config.GetValue<string>("config_Haravan:private_token");
            try
            {
                string sql = await DTOOrder.CreatSqlOrder(obj, token,"1");
                DAL.DAL_SQL.ExecuteNonquery(sql);
            }
            catch (Exception e)
            {
        ...
        public async void UpdateTonKho(JObject obj)
        {
            ...
            try
            {
                ...
                string sql = $"exec haravan_getTonKho_UpdateHaravan '{lst_sp}','1'";
                DataSet ds = DAL.DAL_SQL.ExecuteGetlistDataset(sql);
