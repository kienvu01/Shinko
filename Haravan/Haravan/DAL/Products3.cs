...
namespace Haravan.Haravan.DataAcess
{
    public class Products3
    {
        ...
        public static async Task<ResponseData> CreatNewProduct(string ma_vt)
        {
            ...
            if (ds_getitem.Tables[0].Rows[0]["har_product3"].ToString().Trim() != "" && ds_getitem.Tables[0].Rows[0]["har_variant3"].ToString().Trim() != "") return new ResponseData("err", $"Sản phẩm đã được đồng bộ", "");
            try
            {
                ...
                if (res.status == "ok")
                {
                    ...
                    string sql_gettonkho = Environment.NewLine + $"update dmvt set har_variant3='{new_var_id}',har_product3='{product_id}' where ma_vt='{barcode}'";

                    sql_gettonkho += Environment.NewLine + $"select isnull(sum(isnull(ton13,0)),0) ton13 from cdvt213 where ma_kho='{MyAppData.ma_kho_SSE_MAWO}' and ma_vt='{barcode}'";
        ...
        public static async Task<ResponseData> UpdateProduct(Req_UpdateHaravan data)
        {
            try
            {
                ...
                foreach (Req_updateHaravan_product item in data.items)
                {
                    ...
                    if (ds.Tables[0].Rows[0]["har_variant3"].ToString().Trim() == "") continue;

                    string har_product = "";
                    string har_variant = "";
                    har_product = ds.Tables[0].Rows[0]["har_product3"].ToString();
                    har_variant = ds.Tables[0].Rows[0]["har_variant3"].ToString();
        ...
        public static async Task<ResponseData> DeleteProduct(string ma_vt)
        {
            try
            {
                ...
                if (ds.Tables[0].Rows[0]["har_product3"].ToString().Trim() == "") new ResponseData("err", $"Sản phẩm chưa được đồng bộ", "");

                string har_variant = ds.Tables[0].Rows[0]["har_variant3"].ToString();
                url = $"https://apis.haravan.com/com/variants/{har_variant}.json ";
        ...
        public static async Task<ResponseData> CreatAllNewProduct(List<ProductToHaravanAll> lst_newproduct)
        {
            ...
            {
                tt++;
                string sql = $"exec haravan_GetAllItemToHaravan '{p.ma_vt}',2 ";
                DataSet ds_getitem = DAL.DAL_SQL.ExecuteGetlistDataset(sql);
                string ten_sp = "";
                string ten_locai_vt = "";
                ...
                if (res.status == "ok")
                {
                    ...
                    for (int i = 0; i < listVarianCreat.Count; i++)
                    {
                        ...
                        string new_var_id = new_var["id"].ToString();
                        string barcode = new_var["barcode"].ToString();
                        sql_update += Environment.NewLine + $"update dmvt set har_variant3='{new_var_id}',har_product3='{product_id}' where ma_vt='{barcode}'";
        ...
        public async void UpdateStore24()
        {
            ...
            try
            {
                ...
                string locationId = MyAppData.config.GetValue<string>("config_Haravan3:store024");
                ...
                while (checkss)
                {
                    ...
                    for (int i = 0; i < ListItem.Count; i++)
                    {
                        ...
                        for (int j = 0; j < listvarian.Count; j++)
                        {
                            ...
                            if (ma_vt != null && ma_vt != "")
                            {
                                sql += Environment.NewLine + $"Update dmvt set har_product3='{pro_id}',har_variant3='{varian_id}' where ma_vt='{ma_vt}'";
