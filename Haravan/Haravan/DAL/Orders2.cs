...
namespace Haravan.Haravan.DataAcess
{
    public class Orders2
    {
        ...
        {
            string token = MyAppData.config.GetValue<string>("config_Haravan2:private_token");
            try
            {
                string sql = await DTOOrder.CreatSqlOrder(obj, token,"2");
                DAL.DAL_SQL.ExecuteNonquery(sql);
            }
            catch (Exception e)
            {
        ...
        public async void UpdateTonKho(JObject obj)
        {
            ...
            try
            {
                ...
                string sql = $"exec haravan_getTonKho_UpdateHaravan '{lst_sp}','2'";
                DataSet ds = DAL.DAL_SQL.ExecuteGetlistDataset(sql);
            ...
            try
            {
                ...
                if (checksse.status == "ok")
                {
                    if (!checksse.check)
                    {
                        ...
                        or.UpdateTonKho(obj);
                        //Orders3 or3 = new Orders3();
                        //or3.UpdateTonKho(obj);
