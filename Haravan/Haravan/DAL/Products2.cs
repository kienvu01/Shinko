...
namespace Haravan.Haravan.DataAcess
{
    public class Products2
    {
        ...
        public static async Task<ResponseData> CreatNewProduct(string ma_vt)
        {
            ...
            var token = MyAppData.config.GetValue<string>("config_Haravan2:private_token");
            ...
            if (ds_getitem.Tables[0].Rows[0]["har_product2"].ToString().Trim() != "" && ds_getitem.Tables[0].Rows[0]["har_variant2"].ToString().Trim() != "") return new ResponseData("err", $"Sản phẩm đã được đồng bộ", "");
            try
            {
                ...
                else
                {
                    ...
                    string pro_id = ds_getitem.Tables[1].Rows[0]["har_product2"].ToString();
                    CreatNewItem_Varian var_temp = new CreatNewItem_Varian();
                ...
                if (res.status == "ok")
                {
                    ...
                    string sql_gettonkho = Environment.NewLine + $"update dmvt set har_variant2='{new_var_id}',har_product2='{product_id}' where ma_vt='{barcode}'";
                    ...
                    Object body_kho = new
                    {
                        inventory = new
                        {
                            location_id = MyAppData.ma_kho_Haravan2,
                            type = "set",
                            reason = "shrinkage",
                            note = "Update by SSE",
        ...
        public static async Task<ResponseData> UpdateProduct(Req_UpdateHaravan data)
        {
            try
            {
                ...
                foreach (Req_updateHaravan_product item in data.items)
                {
                    ...
                    if (ds.Tables[0].Rows[0]["har_variant2"].ToString().Trim() == "") continue;

                    string har_product = "";
                    string har_variant = "";
                    har_product = ds.Tables[0].Rows[0]["har_product2"].ToString();
                    har_variant = ds.Tables[0].Rows[0]["har_variant2"].ToString();
                    ...
                    if (res2.status == "ok")
                    {
                        url = $"https://apis.haravan.com/com/inventory_locations.json?location_ids={MyAppData.ma_kho_Haravan2}&variant_ids={har_variant}";
                        decimal quantity = 0;
        ...
        public static async Task<ResponseData> DeleteProduct(string ma_vt)
        {
            try
            {
                ...
                if (ds.Tables[0].Rows[0]["har_product2"].ToString().Trim() == "" ) new ResponseData("err", $"Sản phẩm chưa được đồng bộ", "");

                string har_variant = ds.Tables[0].Rows[0]["har_variant2"].ToString();
                url = $"https://apis.haravan.com/com/variants/{har_variant}.json ";
        ...
        public static async Task<ResponseData> CreatAllNewProduct(List<ProductToHaravanAll> lst_newproduct)
        {
            ...
            {
                tt++;
                string sql = $"exec haravan_GetAllItemToHaravan '{p.ma_vt}',1 ";
                DataSet ds_getitem = DAL.DAL_SQL.ExecuteGetlistDataset(sql);
                string ten_sp = "";
                string ten_locai_vt = "";
                ...
                if (res.status == "ok")
                {
                    ...
                    for (int i = 0; i < listVarianCreat.Count; i++)
                    {
                        ...
                        string new_var_id = new_var["id"].ToString();
                        string barcode = new_var["barcode"].ToString();
                        sql_update += Environment.NewLine + $"update dmvt set har_variant2='{new_var_id}',har_product2='{product_id}' where ma_vt='{barcode}'";
        ...
        public async void UpdateStore24()
        {
            ...
            try
            {
                ...
                string locationId = MyAppData.config.GetValue<string>("config_Haravan2:store024");
                ...
                while (checkss)
                {
                    ...
                    for (int i = 0; i < ListItem.Count; i++)
                    {
                        ...
                        for (int j = 0; j < listvarian.Count; j++)
                        {
                            ...
                            if (ma_vt != null && ma_vt != "")
                            {
                                sql += Environment.NewLine + $"Update dmvt set har_product2='{pro_id}',har_variant2='{varian_id}' where ma_vt='{ma_vt}'";
